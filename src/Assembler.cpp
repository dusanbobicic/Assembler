...
map<string, unsigned char> Assembler::instructionOpCode =
    {
        {"halt", 1},
        {"xchg", 2},
        {"int", 3},
...
void Assembler::doFirstPass()
{
    ...
    for (const auto &tokens : assembly)
    {
        ...
        if (tt == TokenType::LABEL)
        {
            ...
            Symbol *p = new Symbol(labelName, currentSectionS, currentSection, locationCounter, Scope::LOCAL, SymbolType::LABEL, 0);
            p->defined = true;
        ...
        case TokenType::EXT:
            ...
            while (!tokenStream.empty())
            {
                ...
                Symbol *p = new Symbol(sy, "und", SectionLabel::UND, locationCounter, Scope::GLOBAL, SymbolType::LABEL, 0);
                symbolTable.addNewSymbol(*p);
        ...
        case TokenType::SECTION:
            ...
            Symbol *s = new Symbol(currentSectionS, currentSectionS, currentSection, locationCounter, Scope::LOCAL, SymbolType::SECTION, 0);
        ...
        case END:
            ...
            break;
        default:
            throw runtime_error("Wrong token at the start of line during first pass!");
            break;
        }
    }
    tryParseAllEqu();
...
void Assembler::parseDirectiveSRun(string directive, queue<string> ts)
{
    ...
    if (directive == ".skip")
    {
        ...
        unsigned char val = getValueOperand(opS, currentSectionS, currentSection);
    ...
    if (directive == ".word")
    {
        while (!ts.empty())
        {
            ...
            TokenType tt = TokenParser::parseToken(opS);
            if (tt != TokenType::OP_DEC)
                throw runtime_error("Invalid operand for .word xD");
...
void Assembler::processEqu(queue<string> ts)
{
    ...
    Symbol *s = new Symbol(tok, currentSectionS, currentSection, locationCounter, Scope::LOCAL, SymbolType::LABEL, 0);
    symbolTable.addNewSymbol(*s); 
...
void Assembler::handleInsOneOperand(string instruction,queue<string> ts,Section &s){
    ...
    locationCounter-=3;//jos jedan zbog opisa operanda
    u_char DATA1;
    int addressOffset=5;
    short insert;
    u_char regs;
...
void Assembler::handleInsTwoOperand(string instruction,queue<string> ts,Section &s){
    ...
    ts.pop();
    locationCounter+=3;
    value=getValueOperand(op1,currentSectionS,currentSection,nmb,reg);
    locationCounter-=3;
    switch(addT){
        case AddressingType::PC_POM:
        addT=REG_DIR;
...
int Assembler::setPCRelRelocation(string symS,string currSecs){
    ...
    if(symbol.sc==Scope::GLOBAL){
        ...
        Relocation *r= new Relocation(locationCounter,RelType::R_386_32,symb);
        tbl->second.relocations.push_back(r);
        return symbol.offset;//offset simbola od pocetka sekcije
    }
}
